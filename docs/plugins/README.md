...
| twoCh | `plugins/twoCh/` | [`twoCh/README.md`](./twoCh/README.md) | 2ch 専用ブラウザ互換の読み書きインターフェース |
...
[ブラウザ]
  ...
  │   X-Turnstile-Session: xxx       ↓ SESSION_KV (turnstile:<id> 読み込み・検証)
  │
  ├─ POST /test/bbs.cgi         → [twoCh Worker]
  │   Cookie: edge-token=UUID        ↓ SESSION_KV (edge_token:<uuid> 読み込み・検証)
  └─ GET /auth/turnstile        → [twoCh Worker]
      (ブラウザが Turnstile を完了すると edge_token がアクティブ化される)
```
...
| 2ch ブラウザ互換インターフェース | twoCh プラグイン (D1 + SESSION_KV 共有) |
