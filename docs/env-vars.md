...
| `ALLOW_BBS_UI_DOMAINS` | *(リダイレクトなし)* | Turnstile 認証後のリダイレクト許可 UI ドメイン (カンマ区切り) |
...
| `DISABLE_TURNSTILE` | | `"true"` で Turnstile 検証をスキップ (ローカル開発用のみ) |
| `TURNSTILE_TOKEN_TTL` | | Turnstile セッション有効期限 (分単位、`0` = 無期限、デフォルト: `525600` = 1年) |
...
| `TURNSTILE_SITE_KEY` | secret | ✅ | | Cloudflare Turnstile サイトキー |
| `TURNSTILE_SECRET_KEY` | secret | ✅ | | Cloudflare Turnstile シークレットキー |
| `TURNSTILE_TOKEN_TTL` | vars | | `525600` (1年) | トークン有効期限 (分単位) |
| `CORS_ORIGIN` | vars | | `*` | 許可する CORS オリジン |
| `REDIRECT_URL` | vars | | *(なし)* | 認証後リダイレクト先 URL |
